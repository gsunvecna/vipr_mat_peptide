...
sub saveNewGenbank {
    ...
    for my $key (keys %$feats_all) {
        ...
        print STDERR "$subname: File #$ct: saved to $outgbfile\n";
    }
    print STDERR "$subname: Saved $ct files\n";
    print STDERR "$subname: Finished, exit\n\n";
...
sub annotate_1gbk {
    ...
    print STDERR "$subname: \$refpolyprots=$#{$refpolyprots} \$n_reffeat=$n_reffeat\n";
    ...
    $debug && print STDERR "$subname:  ".$inseq->accession_number."  \$num_cds = $num_cds\n";
...
#                $debug && print STDERR "$subname: \$set=\n".Dumper($set)."End of \$set\n\n";
    ...
    for my $key (keys %$feats_all) {
        ...
        for my $k2 (@$refcds_ids) {
          ...
          $n += $#{$feats_all->{$key}->{$k2}}; # array $feats_all->{$key}->{$k2} contains CDS as 0th element
        }
    }
    $comment = "MSA returned mat_peptides vs refseq $n/$n_reffeat";
...
sub run_MSA {
    ...
    foreach my $k (keys %$progs) {
            ...
            $debug && print STDERR "$subname: \$err='$err'\n";
            exit(1) if ($err && print STDERR "$subname: \$err='$err'\n");
    }
...
#                  my $s3 = $cds->seq->translate->seq;
                  ...
                  next if (!defined $s3 && print STDERR "$subname: got undef result. Skip \$j=$j acc=$acc\n");
...
#                  if ($s2 ne $s3) {
                  if ($s2 !~ /$s3/ && Annotate_Verify::diff_2str( $s3, $s2) !~ /^L[.]+$/i) {
                    ...
                    print STDERR "$subname: translation tag and translate don't match in CDS of $acc. Skip.\n";
                    print STDOUT "$subname: translation tag and translate don't match in CDS of $acc. Skip.\n";
                    next;
                  }
                  ...
                  $debug && print STDERR "$subname: \$str2=$str2\n";
        ...
        # Returns a SimpleAlign object
                printf STDERR "$subname: \$cds_all has %d genomes:", $#{$cds_all}+1;
                for my $iii ( 0 .. $#{$cds_all}) {
                    print STDERR " $iii:".$cds_all->[$iii]->display_id;
                }
                print STDERR "\n";
                if ($#{$cds_all}<1) {
                    print STDERR "$subname: too few seqs in \$cds_all $#{$cds_all}+1. Skip \$n_set=$n_set\n";
...
#                $debug && print STDERR "$subname: \$aln=\n". Dumper($aln) . "End of \$aln\n\n";
                $debug && print STDERR "$subname: \$aln->gap_char='". $aln->gap_char . "'\n";
...
#                          $debug && print STDERR "$subname: \$ff=$ff \$f=\n". Dumper($f) . "End of $f\n";
                          ...
                          if ( $len_kk > $len_ff ) {
                            ...
                            print STDERR "$subname: replacing ". $feats_all->{$k2}->{$key}->[$ff]->location->to_FTstring . " with ". $feats->{$key}->{$k2}->[$kk]->location->to_FTstring . " \$seen=$seen\n";
                          }
                          last;
                        }
...
#          $inset  = $inseqs->[$i]->[$n_set+1]->[0];
          ...
          $debug && print STDERR "$subname: MSA_annotate_1cds returned \$feats_new=\n". Dumper($feats_new) . "End of \$feats_new\n";
          $debug && print STDERR "$subname: \$feats_all=\n". Dumper($feats_all) . "End of \$feats_all\n";
...
#    $debug && print STDERR "MSA_annotate_1cds: \$refcds=\n". Dumper($refcds) . "End of \$refcds\n";
    ...
    if (!$aln_h) {
        print STDERR "$subname: null alignment for \$refcds_id='$refcds_id', quit";
        exit;
    }
